...
class OaiPmhRepository_ResponseGenerator extends OaiPmhRepository_OaiXmlGeneratorAbstract {
    ...
    public function __construct($query) {
        ...
        $this->document->registerNodeClass('DOMElement', 'OaiPmhRepository_DOMElement');
        ...
        $responseDate = $this->document->createElement('responseDate', OaiPmhRepository_Date::unixToUtc(time()));
    ...
    protected function _addSetDescription($set, $collection) {
        ...
        $oai_dc->declareSchemaLocation(
                OaiPmhRepository_Metadata_OaiDc::METADATA_NAMESPACE, OaiPmhRepository_Metadata_OaiDc::METADATA_SCHEMA);
    ...
    private function resumeListResponse($token) {
        ...
        if (!$tokenObject || ($tokenObject->verb != $this->query['verb'])) {
            ...
            $this->listResponse($tokenObject->verb, $tokenObject->metadata_prefix, $tokenObject->cursor, $tokenObject->set, $tokenObject->from, $tokenObject->until);
        }
    }
